...
SELECT MAX(salary) FROM hr.employees; -- SELECT MIN(salary) FROM hr.employees;
...
SELECT ROUND(MAX(salary),2) AS maior_salario, ROUND(MIN(salary),2) AS menor_salario, ROUND(SUM(salary),2) AS soma, ROUND(AVG(salary),2) AS media FROM hr.employees;
...
SELECT job_id, AVG(salary), COUNT(*) AS numero_funcionarios FROM hr.employees GROUP BY department_id HAVING numero_funcionarios > 10;
-- 11. 🚀 Escreva uma query que atualize a coluna phone_number, de modo que todos os telefones iniciados por 515 agora devem iniciar com 777.
UPDATE hr.employees
SET phone_number = REPLACE(phone_number, '777%','777.123')
